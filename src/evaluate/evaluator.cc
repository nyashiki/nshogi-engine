...
Evaluator::Evaluator([[maybe_unused]] std::size_t ThreadId,
                     std::size_t FeatureSize, std::size_t BatchSize,
                     infer::Infer* In)
    : PInfer(In)
...
#endif
    FeatureBitboards =
        static_cast<ml::FeatureBitboard*>(allocateMemoryByNumaIfAvailable(
    ...
    WinRate = static_cast<float*>(
        allocateMemoryByNumaIfAvailable(BatchSizeMax * sizeof(float)));
    DrawRate = static_cast<float*>(
        allocateMemoryByNumaIfAvailable(BatchSizeMax * sizeof(float)));

#ifdef CUDA_ENABLED
    // Pin the memory.
    cudaHostRegister(FeatureBitboards,
                     BatchSizeMax * MyFeatureSize * sizeof(ml::FeatureBitboard),
                     cudaHostRegisterDefault);
    cudaHostRegister(Policy, ml::MoveIndexMax * BatchSizeMax * sizeof(float),
                     cudaHostRegisterDefault);
    cudaHostRegister(WinRate, BatchSizeMax * sizeof(float),
                     cudaHostRegisterDefault);
    cudaHostRegister(DrawRate, BatchSizeMax * sizeof(float),
                     cudaHostRegisterDefault);
#endif
}
...
Evaluator::~Evaluator() {
    ...
    freeMemory(reinterpret_cast<void**>(&FeatureBitboards),
               BatchSizeMax * MyFeatureSize * sizeof(ml::FeatureBitboard));
    freeMemory(reinterpret_cast<void**>(&Policy),
               ml::MoveIndexMax * BatchSizeMax * sizeof(float));
    freeMemory(reinterpret_cast<void**>(&WinRate),
               BatchSizeMax * sizeof(float));
    freeMemory(reinterpret_cast<void**>(&DrawRate),
               BatchSizeMax * sizeof(float));
}
...
void Evaluator::freeMemory(void** Memory,
