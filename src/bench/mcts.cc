...
void benchMCTS(uint64_t DurationSeconds, std::size_t BatchSize,
    ...
    allocator::getNodeAllocator().resize(
        (std::size_t)(0.1 * (double)AvailableMemoryGB * 1024 * 1024 * 1024));
    allocator::getEdgeAllocator().resize(
        (std::size_t)(0.9 * (double)AvailableMemoryGB * 1024 * 1024 * 1024));
    ...
    mcts::Manager Manager(NumGPUs, NumThreadsPerGPU, NumCheckmateSearchers,
                          BatchSize, &Logger);
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds(DurationSeconds * 1000));
    Manager.stop();
